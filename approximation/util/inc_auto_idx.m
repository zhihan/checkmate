function next = inc_auto_idx(idx)

% Increment given state index for the global variable "GLOBAL_AUTOMATON"
% by 1.
%
% Syntax:
...
%
% Description:
%   Given the state index "idx" for "GLOBAL_AUTOMATON", increment the
%   index by one and return the result in "next"
%
% Implementation:
%   A state index for "GLOBAL_AUTOMATON" is a row vector of length 2 or
...
%   in "GLOBAL_AUTOMATON", keep incrementing until a valid index is
%   found. If no valid index is found and the location index has gone past
%   the length of the locations in "GLOBAL_AUTOMATON", do the following: 
%   
...
%     last state in "GLOBAL_AUTOMATON". In this case, return an empty
%     matrix "[]".
%
% See Also:
%   inc_new_auto_idx
...
while ~stop
  if (length(idx) == 2) % must be an initial state
    idx(2) = idx(2) + 1;
    is_init_loc = isfield(GLOBAL_AUTOMATON{idx(1)},'initstate');
    if (~is_init_loc) || ...
          (is_init_loc && (idx(2) > length(GLOBAL_AUTOMATON{idx(1)}.initstate)))
      idx(2) = 1;
      idx(1) = idx(1) + 1;
    end
    if (idx(1) > length(GLOBAL_AUTOMATON))
      idx = [1 1 1];
    end
  elseif (length(idx) == 3) % must be a face state
    idx(3) = idx(3) + 1;
    if (idx(3) > length(GLOBAL_AUTOMATON{idx(1)}.face{idx(2)}.state))
      idx(3) = 1;
      idx(2) = idx(2) + 1;
    end
    if (idx(2) > length(GLOBAL_AUTOMATON{idx(1)}.face))
      idx(2) = 1;
      idx(1) = idx(1) + 1;
    end
    if (idx(1) > length(GLOBAL_AUTOMATON))
  ...
  stop = is_valid_auto_idx(idx) | isempty(idx);
end
next = idx;
