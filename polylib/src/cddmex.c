...
/* FT_get_H_MatrixPtr 
//
// Get a matrix from the input mxArray
//   - Copy the A matrix and b matrix into the aggregated matrix [b, -A].
//   - Copy the linearity indices into the data structure.
*/
...
hull(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    if (nrhs  == 1 && nlhs <= 2 && mxIsStruct(prhs[0])) {
        ...
        if (err == dd_NoError) {
            ...
            GI=dd_CopyIncidence(P);
            GA=dd_CopyInputAdjacency(P);
